...
COURSERA_TOKEN = "q3B5mINCkRkwbkvR"# the key provided to the Student under his/her email on submission page
COURSERA_EMAIL = "francisco.palop@gmail.com"# the email
...
class Tobit_Regression:
    ...
    def __init__(self, n_features, learning_rate=0.005, L=0):
        ...
        print("non-augmented data matrix")
        print(self.target)
        ...
        print("augmented data matrix")        
        print(self.data_plus_bias)
        ...
        import math as m
        self.gauss = tf.distributions.Normal(loc=0.0, scale=1.0)
        
        c_zero = tf.constant(0.0)
        c_one = tf.constant(1.0)
        #pi = tf.constant(m.pi) #possibly comment
        
        self.XBeta = tf.matmul(self.data_plus_bias[:, :], self.weights[:-1]) #possibly comment
        
        
        term1 = tf.math.maximum(c_zero, tf.sign(self.target))*tf.log(0.00001 + self.gauss.prob((self.target - self.XBeta) / self.sigma)/self.sigma)
        term2 = (c_one -tf.math.maximum(c_zero, tf.sign(self.target)))*tf.log(0.00001 + c_one - self.gauss.cdf(self.XBeta / self.sigma ))
